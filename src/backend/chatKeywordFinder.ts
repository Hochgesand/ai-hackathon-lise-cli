...
export async function preparedTableOfContents(): Promise<string[]> {
  ...
  const gpt4Model = new OpenAIApi(getConfig("gpt35-turbo"));
  const eventsEmbeddings = await adaModel.createEmbedding({
    model: "text-embedding-ada-002",
    input: "Gib mir ein Inhaltsverzeichnis über deine Daten.",
  });

  const queryInhaltsverzeichnisEmbedding =
    eventsEmbeddings.data.data[0].embedding;
  ...
  const tableOfContent = await gpt4Model.createChatCompletion({
    ...
    messages: [
      ...
      },
      {
        role: "user",
        content:
          "Welche Daten hast du, gebe nur Infos aus die deiner Meinung nach auch Suchanfragen sind. " +
  ...
  return Promise.resolve(
    tableOfContent.data.choices[0].message?.content.split(", ") ?? []
  );
}
...
export async function gptRequest(prompt: string): Promise<GptResult> {
    ...
    messages: [
      {
        role: "user",
        content:
          "Suche mir alle Nomen aus dem Text raus die deiner Meinung nach auch Keywords sind. Bitte gebe mir die Liste kommasepariert aus: " +
          "\nArticles\n" +
          articles.join("\n"),
      },
    ],
  ...
  const newKeywordsArray =
    newKeywords.data.choices[0].message?.content.split(", ");
